...
export class MdmService {
	...
	private getMdmConfigPath(): string {
		...
		switch (platform) {
			case "win32": {
				// Windows: %ProgramData%\ForgeFox\mdm.json or mdm.dev.json
				const programData = process.env.PROGRAMDATA || "C:\\ProgramData"
				return path.join(programData, "ForgeFox", configFileName)
			}

			case "darwin":
				// macOS: /Library/Application Support/ForgeFox/mdm.json or mdm.dev.json
				return `/Library/Application Support/ForgeFox/${configFileName}`

			case "linux":
			default:
				// Linux: /etc/forgefox/mdm.json or mdm.dev.json
				return `/etc/forgefox/${configFileName}`
		}
	}
