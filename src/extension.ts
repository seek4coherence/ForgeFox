...
import type { CloudUserInfo, AuthState } from "@forgefox/types"
import { CloudService } from "@forgefox/cloud"
import { TelemetryService, PostHogTelemetryClient } from "@forgefox/telemetry"
import { customToolRegistry } from "@forgefox/core"
...
/**
 * Check if we should auto-open the ForgeFox sidebar after switching to a worktree.
...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			outputChannel.appendLine(`[Worktree] Auto-opening ForgeFox sidebar for worktree: ${worktreeAutoOpenPath}`)

			// Open the ForgeFox sidebar with a slight delay to ensure UI is ready
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("forgefox.plusButtonClicked")
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	// Initialize the provider *before* the ForgeFox Cloud service.
	const provider = new ClineProvider(context, outputChannel, "sidebar", contextProxy, mdmService)

	// Initialize ForgeFox Cloud service.
	...
	// Implements the `ForgeFoxAPI` interface.
	const socketPath = process.env.FORGEFOX_IPC_SOCKET_PATH
	...
	if (process.env.NODE_ENV === "development") {
		const watchPaths = [
			...
			{ path: path.join(context.extensionPath, "node_modules/@forgefox/cloud"), pattern: "**/*" },
		]
