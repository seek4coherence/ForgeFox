...
import { type ModelInfo, type ModelRecord, unboundDefaultModelId, unboundDefaultModelInfo } from "@forgefox/types"
...
export class UnboundHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: ApiHandlerOptions) {
		...
		this.client = new OpenAI({
			...
			defaultHeaders: {
				...DEFAULT_HEADERS,
				"X-Unbound-Metadata": JSON.stringify({ labels: [{ key: "app", value: "forgefox" }] }),
			},
		})
	}
	...
	override async *createMessage(
		...
		const completionParams: UnboundChatCompletionParamsStreaming = {
			...
			unbound_metadata: { originApp: "forgefox", taskId: metadata?.taskId, mode: metadata?.mode },
