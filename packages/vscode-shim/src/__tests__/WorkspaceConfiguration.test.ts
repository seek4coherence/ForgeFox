...
describe("MockWorkspaceConfiguration", () => {
	...
	describe("Runtime Configuration", () => {
		...
		it("should return runtime config value over disk-based values", async () => {
			const config = new MockWorkspaceConfiguration("forgefox", context)
			...
			setRuntimeConfig("forgefox", "commandExecutionTimeout", 20)
		...
		it("should set and get runtime config values", () => {
			setRuntimeConfig("forgefox", "testSetting", "testValue")

			expect(getRuntimeConfig("forgefox.testSetting")).toBe("testValue")
		})

		it("should set multiple runtime config values at once", () => {
			setRuntimeConfigValues("forgefox", {
				setting1: "value1",
				setting2: 42,
				setting3: true,
			})

			expect(getRuntimeConfig("forgefox.setting1")).toBe("value1")
			expect(getRuntimeConfig("forgefox.setting2")).toBe(42)
			expect(getRuntimeConfig("forgefox.setting3")).toBe(true)
		})
		...
		it("should ignore undefined values in setRuntimeConfigValues", () => {
			...
			expect(getRuntimeConfig("forgefox.defined")).toBe("value")
			expect(getRuntimeConfig("forgefox.notDefined")).toBeUndefined()
		})

		it("should clear all runtime config values", () => {
			setRuntimeConfig("forgefox", "setting1", "value1")
			setRuntimeConfig("forgefox", "setting2", "value2")

			clearRuntimeConfig()

			expect(getRuntimeConfig("forgefox.setting1")).toBeUndefined()
			expect(getRuntimeConfig("forgefox.setting2")).toBeUndefined()
		})
