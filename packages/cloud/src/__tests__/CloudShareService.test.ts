...
import type { SettingsService, AuthService } from "@forgefox/types"
...
vi.mock("../Config", () => ({
	getForgeFoxApiUrl: () => "https://app.forgefox.com",
}))

vi.mock("../utils", () => ({
	getUserAgent: () => "ForgeFox 1.0.0",
}))
	...
	describe("shareTask", () => {
		it("should share task with organization visibility and copy to clipboard", async () => {
			const mockResponseData = {
				success: true,
				shareUrl: "https://app.forgefox.com/share/abc123",
			}
			...
			expect(result.shareUrl).toBe("https://app.forgefox.com/share/abc123")

			expect(mockFetch).toHaveBeenCalledWith("https://app.forgefox.com/api/extension/share", {
				method: "POST",
				headers: {
					"Content-Type": "application/json",
					Authorization: "Bearer session-token",
					"User-Agent": "ForgeFox 1.0.0",
				},
				body: JSON.stringify({
					taskId: "task-123",
			...
			expect(vscode.env.clipboard.writeText).toHaveBeenCalledWith("https://app.forgefox.com/share/abc123")
		})
