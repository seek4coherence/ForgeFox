...
import { useForgeFoxCloudModels } from "@/hooks/use-forgefox-cloud-models"
...
export function NewRun() {
	...
	const forgeFoxCloud = useForgeFoxCloudModels()
	const models = provider === "openrouter" ? openRouter.data : forgeFoxCloud.data
	const searchValue = provider === "openrouter" ? openRouter.searchValue : forgeFoxCloud.searchValue
	const setSearchValue = provider === "openrouter" ? openRouter.setSearchValue : forgeFoxCloud.setSearchValue
	const onFilter = provider === "openrouter" ? openRouter.onFilter : forgeFoxCloud.onFilter
	...
	const onSubmit = useCallback(
		async (values: CreateRun) => {
			try {
				...
				// Validate jobToken for ForgeFox Cloud provider
				if (provider === "roo" && !baseValues.jobToken?.trim()) {
					toast.error("ForgeFox Cloud Token is required")
					return
				}
							...
							<FormItem>
								<Tabs
									...
									<TabsList className="mb-2">
										...
										<TabsTrigger value="roo">ForgeFox Cloud</TabsTrigger>
							...
							render={({ field }) => (
								<FormItem>
									<div className="flex items-center gap-1">
										<FormLabel>ForgeFox Cloud Token</FormLabel>
										<Tooltip>
											...
											<TooltipContent side="right" className="max-w-xs">
												<p>
													If you have access to the ForgeFox Cloud repository and the
													decryption key for the .env.* files, generate a token with:
												</p>
												<code className="text-xs block mt-1">
													pnpm --filter @forgefox-cloud/auth production:create-auth-token
													[email] [org] [ttl]
												</code>
											</TooltipContent>
										</Tooltip>
