...
import { type TokenUsage, type ToolUsage, ForgeFoxEventName, taskEventSchema } from "@forgefox/types"
import type { Run, Task, TaskMetrics } from "@forgefox/evals"
...
export const useRunStatus = (run: Run): RunStatus => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case ForgeFoxEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case ForgeFoxEventName.TaskTokenUsageUpdated: {
			...
			case ForgeFoxEventName.EvalPass:
			case ForgeFoxEventName.EvalFail:
