...
This document explains how the ForgeFox CLI detects and tracks the agent loop state.
...
The CLI uses type guards from `@forgefox/types` for categorization:

```typescript
import { isIdleAsk, isInteractiveAsk, isResumableAsk, isNonBlockingAsk } from "@forgefox/types"
