...
suite.skip("ForgeFox Subtasks", () => {
	test("Should handle subtask cancellation and resumption correctly", async () => {
		const api = globalThis.api

		const messages: Record<string, ClineMessage[]> = {}

		api.on(ForgeFoxEventName.Message, ({ taskId, message }) => {
		...
		api.on(ForgeFoxEventName.TaskSpawned, (_, childTaskId) => (spawnedTaskId = childTaskId))
